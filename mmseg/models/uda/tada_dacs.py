...
@UDA.register_module()
class TADA_DACS(UDADecorator):

    def __init__(self, **cfg):
        super(TADA_DACS, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        with torch.no_grad():
            mixed_ema_features = self.get_ema_model().backbone(mixed_img)
        
        # Inject EMA target features into the student's neck
        if hasattr(self.get_model(), 'neck'):
            self.get_model().neck.ema_target_features = [f[:batch_size].detach() for f in mixed_ema_features]
